...
{
    public static class PhoneIconProvider
    {
        ...
        public static Bitmap GetIcon(Phone phone, bool isOnline)
        {
            var isLight = ThemeHelper.CurrentMode == ThemeMode.Light;

            if (!isOnline)
                return SvgRenderer.RenderSvgFromString(phoneOffline, 25, 25, isLight ? Color.Gray : Color.DarkGray);

            if (phone.IsRunning)
                return SvgRenderer.RenderSvgFromString(phoneIsRuning, 25, 25, isLight ? Color.Orange : Color.Gold);

            return SvgRenderer.RenderSvgFromString(phoneOnline, 25, 25, isLight ? Color.Green : Color.Lime);
        }
    }
}
